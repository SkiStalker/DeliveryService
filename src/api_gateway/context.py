...
async def connect_to_grpc_account(app: FastAPI):
    app.state.account_grpc_channel = grpc.aio.insecure_channel(
        f"{os.environ.get("ACCOUNT_SERVICE_HOST", "localhost")}:{os.environ.get("ACOUNT_SERVICE_PORT", 50051)}"
    )
...
async def connect_to_grpc_user(app: FastAPI):
    app.state.user_grpc_channel = grpc.aio.insecure_channel(
